...
class InternalEncryptBenchmark extends BenchmarkHelper {
  ...
  val encryptInstance = new InternalApi(
    Sha256Hash { (bytes: ByteVector) =>
      ByteVector.view(
        java.security.MessageDigest.getInstance("SHA-256").digest(bytes.toArray)
      )
    },
    Ed25519Signing((_, _) => Signature.empty, (_, _, _) => true),
    Fp.curvePoints
  )
  private val privateKey = PrivateKey.fromBigInt(
    BigInt(
      "17561965855055966875289582496526669116201409974621952158489640859240156546764"
    )
  )
  ...
  private val plaintext =
    encryptInstance.generateRthRoot(randomFp12IO.unsafeRunSync())
  private val publicKey = keyGen(privateKey)
  private val emphemPrivateKey =
    PrivateKey.fromBigInt(BigInt("7982134798234178911111"))
  private val encryptedData = encryptInstance.encrypt(
    publicKey,
    plaintext,
    emphemPrivateKey,
    publicSigningKey,
  ...
  private val salt1 =
    encryptInstance.generateRthRoot(randomFp12IO.unsafeRunSync())
  private val reencryptionPrivate = PrivateKey.fromBigInt(
    BigInt(
      "22002131259228303741090495322318969764532178674829148099822698556219881568451"
    )
  )
  private val raRePrivateKey = PrivateKey.fromBigInt(
    BigInt(
  ...
  private val reencryptedValue = encryptInstance
    .reencrypt(
      reencryptionKey,
      encryptedData,
      raRePrivateKey,
  ...
  @Benchmark
  def encryptLevel0 = encryptInstance.encrypt(
    publicKey,
    plaintext,
    emphemPrivateKey,
    publicSigningKey,
  ...
  @Benchmark
  def reencryptToLevel1 = encryptInstance.reencrypt(
    reencryptionKey,
    encryptedData,
    raRePrivateKey,
    raReK,
  ...
  @Benchmark
  def reencryptToLevel2 = encryptInstance.reencrypt(
    reencryptionKey,
    reencryptedValue,
    privateKey,
    salt1,
